...
Algorithm Created By: [Larry Page](https://en.wikipedia.org/wiki/Larry_Page) and [Sergey Brin](https://en.wikipedia.org/wiki/Sergey_Brin)  
...
![corpus1](https://cs50.harvard.edu/ai/2020/projects/2/pagerank/images/corpus.png)
...
![recursiveFormula](https://latex.codecogs.com/svg.latex?%5Cbg_white%20PR%28p%29%3D%5Cfrac%7B1-d%7D%7BN%7D&plus;d%5Csum%20_i%20%7B%5Cfrac%7BPR%28i%29%7D%7BNumLinks%28i%29%7D%7D)
...
`pagerank.py`:
- `DAMPING`: represents the damping factor and is initially set to `0.85`. 
...
- `main`: Expects command line argument which is the name of a directory of a corpus of web pages we'd want to compute PageRanks for.
- `crawl`: Takes that directory, parses all of the HTML files in the directory, and returns a dictionary representing the corpus.
...
To use `corpus0`:  
`python pagerankpy corpus0`

To use `corpus1`:  
`python pagerankpy corpus1`

To use `corpus2`:  
`python pagerankpy corpus2`
